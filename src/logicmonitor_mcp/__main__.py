...
def _build_http_app(mcp, settings):
    ...
    async def health(_: Request) -> JSONResponse:
        return JSONResponse({"status": "ok", "service": "logicmonitor-mcp", "transport": "http"})
...
def main() -> None:
    ...
    print(
        f"LogicMonitor MCP server listening on http://{settings.mcp_http_host}:{settings.mcp_http_port}/mcp",
        file=sys.stderr,
    )
