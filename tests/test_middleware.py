...
from logicmonitor_mcp.__main__ import _build_http_app
from logicmonitor_mcp.config import Settings
from logicmonitor_mcp.server import create_mcp_server, get_client_from_context
...
def test_missing_header_returns_401_with_required_headers_listed():
    ...
    with TestClient(app) as client:
        ...
        body = resp.json()
        assert "X-LogicMonitor-Company" in body["required_headers"]
        assert "X-LogicMonitor-Access-Id" in body["required_headers"]
        assert "X-LogicMonitor-Access-Key" in body["required_headers"]


def test_partial_headers_still_returns_401():
    # Only one of the three required headers present — must still be 401,
    # not silently proceed with a partial credential tuple.
    app, _ = _make_app()
    with TestClient(app) as client:
        resp = client.post(
            "/mcp",
...
def test_header_present_reaches_request_context(monkeypatch):
    ...
    from logicmonitor_mcp.server import GatewayTokenMiddleware, _gateway_creds_var
    ...
    async def run():
        scope = {
            ...
            "path": "/mcp",
            "headers": [
                (b"x-logicmonitor-company", b"acme"),
                (b"x-logicmonitor-access-id", b"test-id-123"),
                (b"x-logicmonitor-access-key", b"test-key-456"),
            ],
        }
    ...
    asyncio.run(run())
    assert seen["creds"] == ("acme", "test-id-123", "test-key-456")
